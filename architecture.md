...
accepts an optional `--persist` flag, used by the in-game popup (Phase
3b.3) to write the new toggle state back to `bridge/startup.conf`; the
`cp` aliases invoke it without `--persist`, so they remain runtime-only
and never modify `startup.conf`.
