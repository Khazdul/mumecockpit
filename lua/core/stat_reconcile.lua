...
-- This module emits `affects_observed` with the collected name list; the
-- affect tracker reconciles its state from there.
--
...
local function _finish()
    local observed = _buffer
    _buffer = {}
    _active = false
    session_cmd("#unaction {^%1$}")
    events.emit("affects_observed", observed)
end
