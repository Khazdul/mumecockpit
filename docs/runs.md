...
event handler is a no-op when the variable is unset. The timestamp prefix is
generated by an inline `#format _ts {%U}` in the event body, not via a
function: function indirection caused tt++ to reuse the same `%U` evaluation
across all events in a tight received batch, breaking per-line resolution.
