...
the same washed-out bg-tinted shade the UI pane uses for its base-text ink —
`pane_frame.dark_ink()` (`l=40`), a faded ink tinted toward the live terminal
background. Sharing `dark_ink()` means the clock and the UI base text track one
washed-out shade and tune together. The input pane has no pane fill of its own, so
