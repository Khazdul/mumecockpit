...
- [ADR 0138](0138-character-pane-palette-gauges.md) — Character-pane palette-derived stepped gauges — `pane_frame.pane_shades` walks one hue (the pane colour's `(h,s)`, or `terminal_bg`'s under "None") down a lightness ramp (`track`/`dim`/`mid`/`paneBg`/`vtext`/`label`/`glow`); two-level semantics (`track`=off, `glow`=active) retint the XP/TP bars, filled toggle boxes, and a 2×2 stepped-gauge block (mood/alertness/position as `▀` ticks, wimpy as a continuous caret) that replace the old fixed magenta/teal/gold and flat `LABEL: value` rows. `_two_cols` aligns the gauge gap with the caps gap; serialises `maxhp`; content 6→9 rows. (relates to 0086, 0099, 0136, 0126, 0023, 0071)
...
- [ADR 0139](0139-group-pane-display-filter-renderer-side.md) — Group-pane display filter is renderer-side — `group_pane.py`'s `_displayed_members()` draws a user-controlled subset (Show players / NPC visibility, via `group_show_players` / `group_npc_mode`) of `state.group.members`; the collector's canonical membership (ADR 0094/0095) and the state file are unchanged. Revises 0094's "the pane does not filter" note. (relates to 0094, 0095, 0096)
- [ADR 0140](0140-unlabeled-npcs-for-all-mode.md) — Unlabeled group-NPCs for the `all` display mode — `_excluded` is exposed read-only as `state.group.unlabeled` and serialised by `group_state.lua` as a separate `unlabeled_npcs` array (never merged into `members`); `group_pane.py` appends it only when `group_npc_mode == "all"`, and an excluded-only `Group.Remove` now fires `group_changed`. Revises 0095 (`_excluded` no longer private; excluded-only removal no longer silent). (builds on 0139, relates to 0094, 0095)
...
- [ADR 0144](0144-pty-coalescing-output-pump.md) — PTY-coalescing output pump for the game pane — an optional pty wrapper (`bridge/launcher/pty_coalesce.py`, gated by `MUME_COALESCE`) spawns tt++ on a fresh pty and batches its per-line output by a sub-ms read-idle debounce (bounded by a size cap and max-hold ceiling) into one `write()` per burst, killing the mid-burst partial frame; input is immediate byte-for-byte pass-through. Batches by *timing* only, never by content, so it upholds 0050's no-per-line-processing-on-the-hot-path rule. DEC 2026 synchronized-output wrapping is retained behind `MUME_COALESCE_SYNC` but defaults off (no observable benefit on foot 1.16 / tmux 3.4). (relates to 0050, 0103, 0104)
