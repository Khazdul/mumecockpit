...
| Options page | Toggle Character pane / Comm pane / UI / Dev panes; Pane dividers; connection mode; live layout mockup (updates on toggle). Fresh-install defaults: status, comm, ui on; dev off. Content hides progressively at small heights: descriptions → mockup → section headings; menu items always render |
