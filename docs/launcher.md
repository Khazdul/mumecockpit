...
| Options page | Toggle Character pane / Buffs pane / Group pane / Comm pane / UI / Dev panes; Pane dividers; connection mode; live layout mockup (updates on toggle). Fresh-install defaults: status, buffs, comm, ui on; group, dev off. Content hides progressively at small heights: descriptions → mockup → section headings; menu items always render |
