...
| `terminal_bg`    | (probed) | Host terminal background as `#rrggbb`, detected pre-tmux by `bridge/layout/detect_terminal_bg.sh` via OSC 11 and consumed by `bridge/layout/apply_border_style.sh` to blend the inter-pane separator row into the user's terminal theme. Empty when the terminal does not reply within the 0.3 s probe window — borders fall back to `fg=default bg=default`. Written before `build_initial_layout.sh` runs; that script preserves existing keys on (re)creation. |
