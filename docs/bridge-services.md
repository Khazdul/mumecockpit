...
| `input_autosuggest`  | `0`         | Opt-in inline history autosuggestion in the input pane. `1` → as you type, the most recent matching command from this session's history is shown greyed inline after the cursor; Right-arrow / End at end-of-line accepts it into the buffer (not sent until Enter). `0` / absent → off. Toggled from the launcher Options frame (in-place `[X]` / `[ ]` row). Read **once at startup** directly by `bridge/panes/input_pane.py` from `startup.conf` — not via `read_config.sh` or tt++ `#var`, nothing on the hot path; a change takes effect on the next cockpit start. |
