...
| `terminal_bg`    | (probed) | Host terminal background as `#rrggbb`, detected by `bridge/launcher/launcher.py` via OSC 11 at launcher startup (before prompt_toolkit owns the tty). Consumed by `bridge/layout/apply_border_style.sh` to blend the inter-pane separator row into the user's terminal theme, and by the launcher itself for the credits canvas and the spotlight info-box outline. Empty when stdin is not a tty or the terminal does not reply within the ~0.25 s probe window — borders fall back to `fg=default bg=default` and the launcher chrome falls back to a black backdrop / black outline. Not written on the `--no-menu`/`-d`/`-u` paths (launcher is skipped); same fallback applies. `build_initial_layout.sh` only seeds missing keys, so this value survives layout-conf (re)creation. |
