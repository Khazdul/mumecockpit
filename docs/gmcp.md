...
Group.Update carries `id` plus any subset of the above fields that changed. Only present fields are merged into the existing member table; absent fields retain their prior value. Freshness inference is applied per vital pair (hp/hp-string, mana/mana-string, mp/mp-string) — see ADR 0052 for the rules.

Group.Remove payload is a **naked JSON integer**, not an object. dkjson decodes it directly to a Lua number, so the handler uses `tonumber(body)` rather than `body.id`.
